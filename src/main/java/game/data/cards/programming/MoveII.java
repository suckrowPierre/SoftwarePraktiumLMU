...
public class MoveII extends Card {
    ...
    public MoveII() {
        cardName = CardName.MoveII;
        cardDescription = "Move your robot two tiles in the direction it is facing.";
    }
    ...
    public void activateCard(ServerConnectedPlayer player) {
        Logger.tag(LoggingTags.game.toString()).info("Activating Card 'MoveII' for player: " + player.getName());
        ...
        switch (lineOfSight) {
            case top -> {
                for (int i = 0; i < Parameter.MOVEII_STEPS_AMOUNT; i++) {
